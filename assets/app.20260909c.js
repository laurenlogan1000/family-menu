...
(function () {
  ...
  function localUpdatedAt() {
    var saved = load(OKEY);
    return (saved.week === WEEK.week && saved.updatedAt) ? saved.updatedAt : '';
  }
  function setOrder(order, iso) {
    saveLocal(OKEY, {
      week: WEEK.week,
      order: order,
  ...
  function applyRemoteState(remote) {
    ...
    // Only clobber local when remote is same age or newer
    if (localAt && remoteAt && localAt > remoteAt) {
      return false;
    }
    if (Array.isArray(remote.order) && remote.order.length === WEEK.order.length) {
      setOrder(remote.order, remoteAt || new Date().toISOString());
    }
    if (remote.scores && typeof remote.scores === 'object') {
      var sstate = load(SKEY);
      Object.keys(remote.scores).forEach(function (dayId) {
        var sc = remote.scores[dayId] || {};
        sstate[dayId] = {
          well: sc.well != null ? String(sc.well) : '',
  ...
  function collectScores() {
    ...
    document.querySelectorAll('.day[data-day]').forEach(function (day) {
      ...
      days[dayId] = {
        ...
        mealId: day.getAttribute('data-meal') || '',
        well: well,
        likes: likes,
        ease: ease
      };
  ...
  function scoresMap() {
    var days = collectScores();
    var out = {};
    Object.keys(days).forEach(function (k) {
      out[k] = { well: days[k].well, likes: days[k].likes, ease: days[k].ease };
    });
    return out;
  }
  ...
  function boot() {
    ...
    fetch(url, { cache: 'no-store' }).then(function (res) {
      ...
      renderAll();
      // Local swap/scores newer than Pages → push again so remote catches up
      if (!applied && localUpdatedAt()) {
